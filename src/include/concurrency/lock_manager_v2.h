...
//define hash for LockManagerV2::LockMode
namespace std {
    template<>
    struct hash<cmudb::LockMode> {
        auto operator()(const cmudb::LockMode &lock_mode) const -> std::size_t {
            return std::hash<int>()(static_cast<int>(lock_mode));
        }
    };
}
 ...
 */
    ...
    public:
        ...
        public:
            /** type alias for iterator of list<shared_ptr<LockRequest>>**/
            using lock_request_iterator = std::list<std::shared_ptr<LockRequest>>::iterator;
            /** List of lock requests for the same resource (table or row) */
            std::list<std::shared_ptr<LockRequest>> request_queue_;
            /** record transaction id to the position of the request_queue_ **/
            std::unordered_map<txn_id_t, lock_request_iterator> txn_lock_request_map_;
         ...
         */
        LockManagerV2() = default;

        void StartDeadlockDetection() {
            enable_cycle_detection_ = true;
            cycle_detection_thread_ = new std::thread(&LockManagerV2::RunCycleDetection, this);
        }
         ...
         */
        auto LockTable(TransactionWith2PL *txn, LockMode lock_mode, const table_oid_t &oid) noexcept(false) -> bool;
         ...
         */
        auto UnlockTable(TransactionWith2PL *txn, const table_oid_t &oid) -> bool;
         ...
         * @return true if the upgrade is successful, false otherwise
         */
        auto LockRow(TransactionWith2PL *txn, LockMode lock_mode, const table_oid_t &oid, const RID &rid) -> bool;
         ...
         * @return true if the unlock is successful, false otherwise
         */
        auto UnlockRow(TransactionWith2PL *txn, const table_oid_t &oid, const RID &rid, bool force = false) -> bool;
        ...
        TransactionManagerWith2PL *txn_manager_;

        /**
         * helper function for testing
         */
        auto getLockQueueSize(const table_oid_t &oid) -> size_t {
            if (table_lock_map_.find(oid) == table_lock_map_.end()) {
                return 0;
            } else {
                auto lock_queue = table_lock_map_[oid];
                std::unique_lock<std::mutex> queue_lock(lock_queue->latch_);
                return lock_queue->request_queue_.size();
            }
        }
    ...
    private:
        ...
        auto UpgradeLockTable(TransactionWith2PL *txn, LockMode lock_mode, const table_oid_t &oid,
                              LockRequestQueue *lock_request_queue) -> bool;

        auto
        UpgradeLockRow(TransactionWith2PL *txn, LockMode lock_mode, const table_oid_t &oid, const RID &rid) -> bool;

        auto AreLocksCompatible(LockMode l1, LockMode l2) -> bool;

        void ValidateLockMode(TransactionWith2PL *txn, LockMode lock_mode);

        auto ValidateLock(LockRequestQueue *lock_queue, LockMode lock_mode) -> bool;
        ...
        auto
        CheckAppropriateLockOnTable(TransactionWith2PL *txn, const table_oid_t &oid, LockMode row_lock_mode) -> bool;
        ...
        std::unordered_map<LockMode, std::unordered_set<LockMode>> lock_compatible_table_ = {
                {LockMode::SHARED,                     {LockMode::SHARED,           LockMode::INTENTION_SHARED}},
                {LockMode::EXCLUSIVE,                  {}},
                {LockMode::INTENTION_SHARED,
                                                       {LockMode::SHARED,           LockMode::INTENTION_SHARED, LockMode::INTENTION_EXCLUSIVE,
        ...
        /** lock upgrade table */
        std::unordered_map<LockMode, std::unordered_set<LockMode>> lock_upgrade_table_ = {
